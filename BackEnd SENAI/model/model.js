...
const userModel = {
    ...
    registerImage: async (imagemBase64,comentario,id) => {
        ...
        try {
            const [result] = await connection.query("INSERT INTO cadastro (livro,comentario,id) VALUES (?,?,?)", [imagemBase64,comentario,id]);
            return result;
        }
       catch (error) {
    ...
    getByLivro: async (email) => {
        ...
        const [result] = await connection.query("SELECT * FROM cadastro WHERE livro=?", [livro])
            .catch(erro => console.log(erro));
        return result
    },
